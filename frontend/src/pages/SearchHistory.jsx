  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm uppercase tracking-[0.24em] text-indigo-600 dark:text-indigo-300">Search activity</p>
      ...
      {error && <div className="mt-6 rounded-2xl border border-red-300 dark:border-red-700 bg-red-50 dark:bg-red-950 p-4 text-sm text-red-700 dark:text-red-200">{error}</div>}

      <section className="mt-6 rounded-[2rem] border border-slate-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-6">
        {history.length > 0 && (
        ...
        {loading ? (
          ...
          <div className="rounded-3xl border border-dashed border-slate-300 dark:border-gray-700 bg-white dark:bg-gray-950 p-10 text-center">
          ...
          <div className="space-y-4">
            {history.map((item) => (
              <div
                key={item.session_id}
                className="w-full rounded-3xl border border-slate-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-5 text-left transition hover:border-indigo-500 hover:bg-slate-50 dark:hover:bg-gray-900"
              >
                <div className="mb-4 flex items-center justify-between gap-3">
                  <label className="flex items-center gap-3 text-sm text-slate-700 dark:text-gray-300">
                    <input
                      ...
                      className="h-4 w-4 rounded border-slate-300 bg-white dark:border-gray-600 dark:bg-gray-900 text-indigo-500 focus:ring-indigo-500"
                    />
                    Select item
                  </label>
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                    <div className="flex items-start gap-3">
                      <FileText size={18} className="mt-1 text-indigo-600 dark:text-indigo-300" />
                      <div>
                        <p className="text-xs uppercase tracking-[0.2em] text-slate-500 dark:text-gray-500">Resume</p>
                        <p className="mt-1 text-sm text-slate-900 dark:text-white">{item.resume_name || item.resume_path || 'Unknown resume'}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      <User size={18} className="mt-1 text-indigo-600 dark:text-indigo-300" />
                      <div>
                        <p className="text-xs uppercase tracking-[0.2em] text-slate-500 dark:text-gray-500">Role</p>
                        <p className="mt-1 text-sm text-slate-900 dark:text-white">{item.role}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      <MapPin size={18} className="mt-1 text-indigo-600 dark:text-indigo-300" />
                      <div>
                        <p className="text-xs uppercase tracking-[0.2em] text-slate-500 dark:text-gray-500">Location</p>
                        <p className="mt-1 text-sm text-slate-900 dark:text-white">{item.location}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      <Clock3 size={18} className="mt-1 text-indigo-600 dark:text-indigo-300" />
                      <div>
                        <p className="text-xs uppercase tracking-[0.2em] text-slate-500 dark:text-gray-500">Search time</p>
                        <p className="mt-1 text-sm text-slate-900 dark:text-white">{new Date(item.created_at).toLocaleString()}</p>
                      </div>
                    </div>
                  </div>
                  <div className="inline-flex items-center gap-2 text-sm font-medium text-indigo-600 dark:text-indigo-300">
                    View results
                    <ArrowRight size={16} />
                  </div>
                </div>
