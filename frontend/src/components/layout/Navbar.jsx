import { Briefcase, ChevronLeft, Moon, Sun } from 'lucide-react'
import { Link, useLocation, useNavigate } from 'react-router-dom'
import { useJobAgent } from '../../hooks/useJobAgent.jsx'
import { Button } from '../ui/Button.jsx'

export function Navbar() {
  const { theme, toggleTheme } = useJobAgent()
    ...
    <header className="sticky top-0 z-50 border-b border-gray-200 bg-white/90 text-slate-900 backdrop-blur-xl transition-colors duration-200 dark:border-gray-800 dark:bg-gray-900/95 dark:text-white">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-4">
        <Link to="/" className="inline-flex items-center gap-3 text-slate-900 dark:text-white">
          <div
            className={[
              'flex h-10 w-10 items-center justify-center rounded-2xl text-white transition-all duration-200',
              theme === 'dark'
                ? 'bg-indigo-600 shadow-lg shadow-indigo-500/20'
                : 'bg-gradient-to-br from-indigo-500 to-violet-600 shadow-md shadow-indigo-500/10 ring-1 ring-slate-200'
            ].join(' ')}
          >
            <Briefcase size={20} />
          </div>
          <span className="text-lg font-semibold tracking-tight text-indigo-700 dark:text-indigo-300">JobAgent</span>
        </Link>
