...
export function JobCard({ job, onDownloadResume, onDownloadCoverLetter, onViewDetail }) {
  return (
    <div className="group rounded-3xl border border-gray-800 bg-gray-950 p-6 transition hover:border-indigo-500/50">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          <h3 className="text-lg font-semibold text-white">{job.title}</h3>
          <p className="mt-1 text-sm text-gray-400">{job.company} · {job.location}</p>
        </div>
        <Badge label={job.status || 'pending'} variant={statusVariant[job.status] || 'new'} />
      </div>
      <p className="mb-4 text-sm leading-6 text-gray-400">{truncateText(job.description, 120)}</p>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        <button
          type="button"
          className="inline-flex items-center justify-center rounded-xl border border-gray-800 bg-gray-900 px-4 py-2 text-sm font-medium text-gray-200 transition hover:border-indigo-500/50 disabled:cursor-not-allowed disabled:opacity-50"
